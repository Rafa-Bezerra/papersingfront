...
export async function getAll(): Promise<AprovadoresFiscal[]> {
    ...
    const list: AprovadoresFiscal[] = await res.json();
    return list;
}

export async function getElementById(id: number): Promise<AprovadoresFiscal> {
    ...
    const apiData: AprovadoresFiscal = await res.json();
    return apiData;
}
