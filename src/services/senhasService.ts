...
export async function updateSenhaExterno(
    payload: AlterarSenha
) {
    const res = await fetch(`${API_BASE}/api/Externos/alterar_senha`, { method: "POST", headers: headersExterno(), body: JSON.stringify(payload) })
    if (!res.ok) {
        const msg = await res.text();
        throw new Error(`Erro ${res.status} ao atualizar senha: ${msg}`);
    }
