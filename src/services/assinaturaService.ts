...
export async function getExterno(): Promise<Assinatura> {
    const res = await fetch(`${API_BASE}/api/Externos/assinatura`, {
...
export async function updateAssinaturaExterno(data: Assinatura): Promise<void> {
    const res = await fetch(`${API_BASE}/api/Externos/assinatura`, { method: "POST", headers: headersExterno(), body: JSON.stringify(data) });
    if (!res.ok) {
      const msg = await res.text();
      throw new Error(`Erro ${res.status} ao atualizar ${elemento_singular}: ${msg}`);
    }
