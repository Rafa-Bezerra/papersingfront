...
export async function getAll(dateFrom: string, dateTo: string, movimentos: string[], situacao: string, restrito: string, situacaoEntrega: string = ""): Promise<RequisicaoDto[]> {
    const url = new URL(`${API_BASE}/api/${caminho}`);

    const body = { dateFrom, dateTo, movimentos, situacao, restrito, situacao_entrega: situacaoEntrega };
