...
import { loginExterno, LoginExternoPayload } from '@/services/auth'
...
export default function LoginPage() {
  ...
  const onSubmit: SubmitHandler<LoginFormValues> = async values => {
    try {
      ...
      const payload: LoginExternoPayload = {
        username: values.usuario,
        password: values.password
      }

      const usuario = await loginExterno(payload)
      
      sessionStorage.setItem('authTokenExterno', usuario.token)
      sessionStorage.setItem('userDataExterno', JSON.stringify(usuario))
      router.push('/documentos-externo')
    } catch (error) {
      const message =
