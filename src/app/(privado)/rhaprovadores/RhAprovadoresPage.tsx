...
import { AprovadoresRh, createElement, deleteElement, getAll, updateElement } from '@/services/aprovadoresRhService'

export default function Page() {
  const titulo = 'Aprovadores de Rh'
  const tituloInsert = 'Novo usuário'
  const router = useRouter()
  const searchParams = useSearchParams()

  const [query, setQuery] = useState<string>(searchParams.get('q') ?? '')
  const [results, setResults] = useState<AprovadoresRh[]>([])
  ...
  const form = useForm<AprovadoresRh>({
    defaultValues: {
      id: 0,
      usuario: '',
      cargo: '',
  ...
  function handleEditar(data: AprovadoresRh) {
    form.reset({
      id: data.id,
  ...
  async function onSubmit(data: AprovadoresRh) {
    setError(null)
    try {
      if (updateMode) {
  ...
  const colunas = useMemo<ColumnDef<AprovadoresRh>[]>(
    () => [
