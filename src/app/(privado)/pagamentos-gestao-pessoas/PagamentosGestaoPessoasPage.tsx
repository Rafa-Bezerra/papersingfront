...
/**
 * TELA: Pagamentos G. Pessoas (/pagamentos-gestao-pessoas)
 * - Pagamentos de folha (salários, férias, 13º, rescisões, FGTS, INSS, IRRF etc) com CI e anexos.
 * - Backend deve filtrar: STATUS_LANCAMENTO/WF_STATUS = EM ABERTO e TIPO_DOCUMENTO = PAGAMENTO_RH.
 * - Modal "Aprovador Gestão Pessoas": mesma API que Contratos (AprovadoresGestaoPessoas).
 * - Após aprovação: UPDATE FLAN.CAMPOALFAOP3 (F/A/C). Relatório: Autorização de Pagamento Financeiro.
 * BACKEND: GET /api/PagamentosGestaoPessoas + GET/POST/PUT/DELETE /api/AprovadoresGestaoPessoas.
 * Ver: src/services/pagamentosService.ts e gestaoPessoasService.ts, types/LancamentoPagamento.ts
 */
...
import { getPagamentosGestaoPessoas } from '@/services/pagamentosService'
import {
  getAprovadores,
  createAprovador,
  updateAprovador,
...
export default function PagamentosGestaoPessoasPage() {
  const titulo = 'Pagamentos G. Pessoas'
  ...
  async function loadData() {
    setError(null)
    try {
      const dados = await getPagamentosGestaoPessoas()
      setResults(dados)
    } catch (err) {
    ...
    <div className="p-6">
      <Card className="mb-6">
        ...
        <CardContent>
          <p className="text-sm text-muted-foreground">
            Pagamentos de folha (EM ABERTO, tipo PAGAMENTO_RH). Hierarquia: Fabrício → GAF → Diretor Unidade → Gomes → Lopes.
          </p>
        </CardContent>
      </Card>
      ...
      {searched && results.length === 0 && !loading && !error && (
        <p className="mt-4 text-center text-sm text-muted-foreground">
          Nenhum pagamento em aberto (PAGAMENTO_RH). Configure o backend para a API PagamentosGestaoPessoas.
        </p>
      )}
    </div>
  )
