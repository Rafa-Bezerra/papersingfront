...
/**
 * TELA: Contratos Gestão de Pessoas (/gestao-pessoas)
 * - Exibe tabela de contratos com coluna STATUS_RESTRICAO (NULL ou RESTRITO). Contratos RESTRITO entram neste fluxo.
 * - Botão "Aprovador Gestão Pessoas" abre modal com CRUD de aprovadores (usuário, cargo, valor inicial/final, nível).
 * BACKEND: GET /api/ContratosGestaoPessoas (lista) + GET/POST/PUT/DELETE /api/AprovadoresGestaoPessoas.
 * Ver comentários em: src/services/gestaoPessoasService.ts e src/types/ContratoGestaoPessoas.ts
 */

import React, { useEffect, useMemo, useRef, useState, useTransition } from 'react'
import { useRouter, useSearchParams } from 'next/navigation'
import { ColumnDef } from '@tanstack/react-table'
import { SearchIcon, SquarePlus, X, Users } from 'lucide-react'
...
export default function GestaoPessoasPage() {
  ...
  async function handleSearch(q: string) {
    setError(null)
    try {
      const dados = await getContratos()
      const qNorm = stripDiacritics(q.toLowerCase().trim())
      const filtrados = qNorm
        ? dados.filter(
            (d) =>
              stripDiacritics((d.nome_fornecedor ?? '').toLowerCase()).includes(qNorm) ||
              stripDiacritics((d.codigo_fornecedor ?? '').toLowerCase()).includes(qNorm) ||
              (d.sao ?? '').toString().includes(qNorm)
          )
        : dados
  ...
  async function openModalAprovadores() {
    setError(null)
    try {
      const dados = await getAprovadores()
  ...
  async function submitAprovador(
    ...
    try {
      if (data.id && data.id !== 0) {
        await updateAprovador(data.id, data)
      } else {
      ...
      const list = await getAprovadores()
      setAprovadores(list)
    } catch (err) {
      toast.error((err as Error).message)
    }
  ...
  const colunas = useMemo<ColumnDef<ContratoGestaoPessoas>[]>(
    () => [
      { accessorKey: 'sao', header: 'SAO' },
      {
        accessorKey: 'valor_total',
        header: 'Valor total',
        cell: ({ row }) =>
          row.original.valor_total != null
            ? Number(row.original.valor_total).toLocaleString('pt-BR', {
                minimumFractionDigits: 2,
              })
            : '-',
      },
      { accessorKey: 'codigo_fornecedor', header: 'Cód. fornecedor' },
      { accessorKey: 'nome_fornecedor', header: 'Nome fornecedor' },
      {
        accessorKey: 'status_restricao',
        header: 'Status restrição',
        cell: ({ row }) => {
          const v = row.original.status_restricao
          return (
            <span
              className={
                v === 'RESTRITO'
      ...
      { accessorKey: 'status_movim', header: 'Status movim.' },
    ],
    []
  )
  ...
  return (
    <div className="p-6">
      <Card className="mb-6">
        <CardHeader className="flex flex-row items-center justify-between">
          ...
          <Button onClick={openModalAprovadores} variant="outline" className="flex items-center gap-2">
            ...
            Aprovador Gestão Pessoas
          </Button>
        </CardHeader>
        <CardContent className="flex flex-col gap-2 md:flex-row">
          <div className="relative flex-1">
            <Input
              placeholder="Pesquisar por fornecedor ou SAO"
              value={query}
      ...
      {/* Dialog confirmar exclusão aprovador */}
      <Dialog
      ...
      {searched && results.length === 0 && !loading && !error && (
        <p className="mt-4 text-center text-sm text-muted-foreground">
          Nenhum contrato encontrado. Configure o backend para a API ContratosGestaoPessoas.
        </p>
      )}
    </div>
  )
