...
import { AprovadoresFinanceiro, createElement, deleteElement, getAll, updateElement } from '@/services/aprovadoresFinanceiroService'

export default function Page() {
  const titulo = 'Aprovadores Financeiro'
  const tituloInsert = 'Novo usuário'
  const router = useRouter()
  const searchParams = useSearchParams()

  const [query, setQuery] = useState<string>(searchParams.get('q') ?? '')
  const [results, setResults] = useState<AprovadoresFinanceiro[]>([])
  ...
  const form = useForm<AprovadoresFinanceiro>({
    defaultValues: {
      id: 0,
      usuario: '',
      cargo: '',
      ...
      const filtrados = qNorm
        ? dados.filter(
          ...
          (
            ...
            String(p.valor_final ?? '').includes(qNorm) ||
  ...
  function handleEditar(data: AprovadoresFinanceiro) {
    form.reset({
      id: data.id,
  ...
  async function onSubmit(data: AprovadoresFinanceiro) {
    setError(null)
    try {
      if (updateMode) {
  ...
  const colunas = useMemo<ColumnDef<AprovadoresFinanceiro>[]>(
    () => [
