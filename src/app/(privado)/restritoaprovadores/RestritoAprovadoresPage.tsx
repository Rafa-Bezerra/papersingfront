...
import { AprovadoresRestritos, createElement, deleteElement, getAll, updateElement } from '@/services/aprovadoresRestritosService'

export default function Page() {
  const titulo = 'Aprovadores de Gestão de Pessoas'
  const tituloInsert = 'Novo usuário'
  const router = useRouter()
  const searchParams = useSearchParams()

  const [query, setQuery] = useState<string>(searchParams.get('q') ?? '')
  const [results, setResults] = useState<AprovadoresRestritos[]>([])
  ...
  function handleEditar(data: AprovadoresRestritos) {
    form.reset({
      id: data.id,
  ...
  async function onSubmit(data: AprovadoresRestritos) {
    setError(null)
    try {
      if (updateMode) {
