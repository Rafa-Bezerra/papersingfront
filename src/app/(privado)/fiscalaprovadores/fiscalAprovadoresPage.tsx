...
import { AprovadoresFiscal, createElement, deleteElement, getAll, updateElement } from '@/services/aprovadoresFiscalService'

export default function Page() {
  const titulo = 'Aprovadores do Painel Fiscal'
  const tituloInsert = 'Novo usuário'
  const router = useRouter()
  const searchParams = useSearchParams()

  const [query, setQuery] = useState<string>(searchParams.get('q') ?? '')
  const [results, setResults] = useState<AprovadoresFiscal[]>([])
  ...
  const form = useForm<AprovadoresFiscal>({
    defaultValues: {
      id: 0,
      usuario: '',
      cargo: '',
  ...
  function handleEditar(data: AprovadoresFiscal) {
    form.reset({
      id: data.id,
  ...
  async function onSubmit(data: AprovadoresFiscal) {
    setError(null)
    try {
      if (updateMode) {
  ...
  const colunas = useMemo<ColumnDef<AprovadoresFiscal>[]>(
    () => [
