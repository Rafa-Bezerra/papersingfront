...
function aplicarResumo(data: PendenciasGestorResponse) {
  return {
    total: data.total,
    totalExibidos: data.totalExibidos ?? data.itens.length,
    porUnidade: unidadesComPendencias(data.porUnidade),
  };
}
...
export default function PendenciasGestorHomeCard() {
  ...
  const aplicarDados = useCallback((data: PendenciasGestorResponse) => {
    const resumo = aplicarResumo(data);
    setTotal(resumo.total);
    setTotalExibidos(resumo.totalExibidos);
    setPorUnidade(resumo.porUnidade);
    setLoading(false);
  }, []);
  ...
  useEffect(() => {
    ...
    const onLogin = () => void carregar(true);
