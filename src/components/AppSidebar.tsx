  ...
  return (
    <>
      ...
      <div className={`relative h-full ${mobileOpen ? 'block' : 'hidden'} lg:block`}>
        ...
        <div className={`sidebar-container relative transition-all duration-300 ease-in-out flex-shrink-0 h-full ${
          collapsed ? 'sidebar-collapsed' : 'sidebar-expanded'
        } border-r bg-card ${mobileOpen ? 'fixed left-0 top-0 z-50 w-64' : ''}`}>
        <div className={`h-full flex flex-col transition-all duration-300 ${
          collapsed ? 'px-1 py-4' : 'px-4 py-4'
        }`}>
          {/* Botão de toggle para desktop - posicionado dentro do sidebar */}
          <div className={`flex mb-2 transition-all duration-300 ${
                    ...
                    ((item.url != "/alcadas" && item.url != "/usuarios") || userAdmin) && (<SidebarMenuItem key={item.title}>
                        <SidebarMenuButton 
                          ...
                          className={`w-full transition-all duration-200 ${
                            collapsed ? 'justify-center px-2 py-2.5 rounded-md' : 'justify-start px-3'
                          } ${
                        ...
                        >
                          ...
                          <div className={`flex items-center ${
                            ...
                            <span className={`flex-shrink-0 transition-colors duration-200 ${
                              path === item.url || path.startsWith(item.url + '/') ? 'text-slate-800 dark:text-white' : 'text-foreground'
                            }`}>
                              {iconMap[item.title] ?? <span className="w-5 h-5" />}
                            </span>
                            {!collapsed && (
                              <span className={`truncate text-sm ${
                                path === item.url || path.startsWith(item.url + '/') ? 'text-slate-800 dark:text-white font-semibold' : 'text-foreground font-medium'
                              }`}>
                                {item.title}
                              </span>
                            )}
